...
export function ViewerInfoPanel({
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-[#0a0a0a]">
        <div className="flex items-center justify-between px-4 h-14 border-b border-white/[0.06] shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-8 w-8 rounded-lg flex items-center justify-center text-white/50 hover:text-white"
            aria-label="Fechar"
          >
            <X className="h-4 w-4" />
          </button>
  ...
  return (
    <aside className="hidden lg:block w-[320px] shrink-0 border-l border-white/[0.06] bg-[#0a0a0a] overflow-y-auto sticky top-0 h-[calc(100vh-64px)] self-start">
      {body}
    </aside>
  );
}
